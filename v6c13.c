    ...
    INITPSF *initStruct)
{
  ...
  if(global->help)
  {
	...
	\n \t \t -i, --interactive \t Force the programme to ignore all other arguments (bar help) \
  ...
  if(global->interactive)
  {
    ...
    global->userInput 		= (char*) malloc(_POSIX_NAME_MAX * sizeof(char));	//Allocates memory for user input using limits defined in limits.h
    ...
    strncpy(global->pattern, "^[[:alnum:]]{1,64}(.wav$|.aif$)", 31 * sizeof(char));	//Regular Expressions used validate user input
    while(inputValid == 0)
    {
